...
ID3D11Buffer       *gpID3D11Buffer_VertexBuffer_Quad_Position = NULL;
...
HRESULT initialize()
{
    ...
    bufferDesc.ByteWidth = sizeof(float)*ARRAYSIZE(vertices_triangle);
    ...
    memcpy(mappedSubResource.pData,vertices_triangle,sizeof(vertices_triangle));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Triangle_Position,NULL);

    //======================Triangle Color============
    ZeroMemory(&bufferDesc,sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * sizeof(color_triangle);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    ...
    memcpy(mappedSubResource.pData,color_triangle,sizeof(color_triangle));
    ...
    ZeroMemory(&bufferDesc,sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * sizeof(vertices_quad);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc,NULL,&gpID3D11Buffer_VertexBuffer_Quad_Position);
    if(FAILED(hr))
    {
        fopen_s(&gpFile,gszLogFileName,"a+");
        fprintf_s(gpFile,"CreateBuffer() Failed \n");
    ...
    ZeroMemory(&mappedSubResource,sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Quad_Position,0,D3D11_MAP_WRITE_DISCARD,0,&mappedSubResource);
    memcpy(mappedSubResource.pData,vertices_quad,sizeof(vertices_quad));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Quad_Position,0);

    //Quad Color
    ZeroMemory(&bufferDesc,sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * sizeof(color_quad);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc,NULL,&gpID3D11Buffer_VertexBuffer_Quad_Color);
    if(FAILED(hr))
    {
        fopen_s(&gpFile,gszLogFileName,"a+");
        fprintf_s(gpFile,"CreateBuffer() Failed \n");
    ...
    ZeroMemory(&mappedSubResource,sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Quad_Color,0,D3D11_MAP_WRITE_DISCARD,0,&mappedSubResource);
    memcpy(mappedSubResource.pData,color_quad,sizeof(color_quad));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Quad_Color,0);
    //=========================================Input Layout=================================
...
void display()
{
    ...
    rotationMatrix = XMMatrixRotationY(triangle_rotation);
    ...
    //Quad
    gpID3D11DeviceContext->IASetVertexBuffers(0,1,&gpID3D11Buffer_VertexBuffer_Quad_Position,&stride,&offset);
    gpID3D11DeviceContext->IASetVertexBuffers(1,1,&gpID3D11Buffer_VertexBuffer_Quad_Color,&stride,&offset);
    gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    ...
    rotationMatrix = XMMatrixRotationX(quad_rotation);
...
void update()
{
    triangle_rotation = triangle_rotation + 0.005f;
    if(triangle_rotation >= 360.0f)
        triangle_rotation = 0.0f;

    quad_rotation = quad_rotation + 0.005f;
    if(quad_rotation >= 360.0f)
        quad_rotation = 0.0f;
}
...
void uninitialize()
{
    ...
    if(gpID3D11Buffer_VertexBuffer_Quad_Color)
    {
        gpID3D11Buffer_VertexBuffer_Quad_Position->Release();
        gpID3D11Buffer_VertexBuffer_Quad_Position = NULL;
    }

    if(gpID3D11Buffer_VertexBuffer_Quad_Position)
    {
        gpID3D11Buffer_VertexBuffer_Quad_Position->Release();
        gpID3D11Buffer_VertexBuffer_Quad_Position = NULL;
    }
